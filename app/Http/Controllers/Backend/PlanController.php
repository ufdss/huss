...
class PlanController extends AbstractController
{
    ...
    {
        parent::__construct($model,'plans');
//        $this->middleware(['permission:backend']);
    ...
    public function datatable()
    {
        return DataTables::of($this->model->query())
            ->addColumn('name', function($row){
                return '<a href="'.route('backend.plans.edit', $row->id).'">'.$row->name.'</a>';
            })
            ->rawColumns(['name'])
            ->make(true);
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('backend.plans.index')->with('doneMessage', __('Created Successfully'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('backend.plans.index')->with('doneMessage', __('Updated Successfully'));
    }
